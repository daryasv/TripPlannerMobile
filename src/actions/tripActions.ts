...
export const GetCities = (country,
  ...
  Axios.post("https://countriesnow.space/api/v0.1/countries/cities", {country: country})
    .then((res) => {
...
export const GetSavedLocations = (city,
...
export const AddLocationToRoute = (data : updateRouteData,
...
export const saveRoute = (data : saveRoutePlan,
  callback: (success: PostType[] | false) => void
) => {
  const headers = {
    'Authorization': "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NGU0OTlhNTQzZWY2YWU2YWQ5MjUyNDYiLCJlbWFpbCI6Im9maXJAZ21haWwuY29tIiwiaWF0IjoxNjkzMDU4MzkzfQ.A68j4XPrFo7mRdtckwbCCo_8wLFjpy_kxPx_kD5UkkM",
...
export const getRegion = (city,
  ...
  Axios.get("https://maps.googleapis.com/maps/api/geocode/json?address=" + city + "&key=" + MAPS_KEY_IOS)
    .then((res) => {
      console.log("success", JSON.stringify(res));
      const data = res.data;
        if (data.results.length > 0) {
          const location = data.results[0].geometry.location;
          callback({ latitude: location.lat, longitude: location.lng, latitudeDelta: 0.0922, longitudeDelta: 0.0421 });
        }
    })
    .catch((e) => {
      console.log("error", JSON.stringify(e));
